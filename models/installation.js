...
class InstallationModel extends Model {
  ...
  update(record) {
    if (record.job_order_id) {
      ...
      const colSet = new pgp.helpers.ColumnSet(['job_order_id'], {table: joinTable});
      const query = pgp.helpers.update({job_order_id: record.job_order_id}, colSet) + ' WHERE installation_id = ' + record.id + ' RETURNING *';
      console.log(query);
  ...
  getById(id) {
    const joinTable = new pgp.helpers.TableName('installation_job_order');
    const joinClause = ' INNER JOIN ${joinTable} on installation_job_order.installation_id = t1.id'
